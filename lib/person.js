...
class Person {
	...
	static create(options) {
		...
		var p = new Person();
		p.sex=options.sex || Gender.Any();
		p.name=options.name||(p.sex==Gender.Female?names.female.rnd().name:names.male.rnd().name);
		p.birthdate=options.birthdate || Person.birthdate(age);
		return p;
	}
	static fromJSON(data) {	
	...
	toJSON() {
		...
		return {
			...
			data: {
				sex: self.sex,
				name: self.name,
				birthdate: self.birthdate.toISOString()
			}		
		}
	}
	inspect() {
